...
function authenticateToken(req, res, next) {
  ...
  jwt.verify(token, SECRET, (err, user) => {
    if (err) {
      return res
        .status(403)
        .json({ status: "FAILED", message: "Forbidden: Invalid token" });
    }
    req.user = user;
    next();
  });
...
router.post("/requestPasswordReset", authenticateToken, (req, res) => {
...
router.post("/resetPassword", authenticateToken, (req, res) => {
